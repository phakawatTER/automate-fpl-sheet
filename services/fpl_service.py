...
class Service:
    def __init__(self,google_sheet:GoogleSheet,config:Config):
        ...
        self.worksheet = self.google_sheet.open_worksheet_from_default_sheet(self.config.worksheet_name)
    ...
    async def update_fpl_table(self,gameweek:int):
        ...
        self.__update_player_score_in_sheet(current_score_col=current_score_col,start_score_row=START_SCORE_ROW,players=players)
    ...
    def __update_players_reward_from_sheet(
        ...
        player_bank_accounts = [t[0] for t in  self.worksheet.get(self.config.player_bank_account_range)]
    ...
    def __update_player_score_in_sheet(
        ...
        for i, player_id in enumerate(player_ids):
            for player in players:
                if player.player_id == int(player_id):
                    current_score_row = start_score_row + i
                    self.worksheet.update_cell(current_score_row,current_score_col, player.score)
                    break
    ...
    def __lookup_gameweek_result_cache(self,game_week:int)-> Optional[List[PlayerResultData]]:
